...
DEFAULT_PROMPTS_FILE = EXPERIMENT_ROOT / "fixtures" / "test_b_numeric_pressure_smoke.json"
DEFAULT_PATCH_DELTA_GRAMMAR_FILE = EXPERIMENT_ROOT / "grammars" / "compact_delta_patch_v2.gbnf"
DEFAULT_PATCH_DELTA_SCHEMA_FILE = EXPERIMENT_ROOT / "schemas" / "compact_delta_patch_v2.json"
DEFAULT_EXPLANATION_GRAMMAR_FILE = EXPERIMENT_ROOT.parent / "Experiment 1.2" / "grammars" / "explanation_response_v1.gbnf"
DEFAULT_EXPLANATION_SCHEMA_FILE = EXPERIMENT_ROOT.parent / "Experiment 1.2" / "schemas" / "explanation_response_v1.json"
DEFAULT_PARAMETER_REGISTRY_FILE = EXPERIMENT_ROOT / "configs" / "parameter_registry_v1.json"

PATCH_CONTRACT_COMPACT = "compact_delta"
PATCH_CONTRACT_HYBRID = "hybrid_explanation_delta"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Experiment 1.3 for compact-delta, hybrid, and compatibility Sound Forge Brain pipelines."
    )
    parser.add_argument(
        "--patch-model-path",
        type=Path,
    ...
    parser.add_argument(
        "--patch-contract",
        choices=[PATCH_CONTRACT_COMPACT, PATCH_CONTRACT_HYBRID],
        default=PATCH_CONTRACT_COMPACT,
    ...
    parser.add_argument(
        "--disable-patch-grammar",
        action="store_true",
        help="Skip patch grammar enforcement for compatibility smoke tests.",
    )
    parser.add_argument(
        "--min-patch-prompt-tokens",
        type=int,
        default=None,
        help="Fail the run if any patch prompt token count falls below this threshold.",
    )
...
def resolve_patch_contract_paths(args: argparse.Namespace) -> tuple[Path, Path]:
    if args.patch_contract == PATCH_CONTRACT_COMPACT:
        default_grammar = DEFAULT_PATCH_DELTA_GRAMMAR_FILE
        default_schema = DEFAULT_PATCH_DELTA_SCHEMA_FILE
    elif args.patch_contract == PATCH_CONTRACT_HYBRID:
        default_grammar = EXPERIMENT_ROOT / "grammars" / "hybrid_explanation_delta_v1.gbnf"
        default_schema = EXPERIMENT_ROOT / "schemas" / "hybrid_explanation_delta_v1.json"
    else:
        raise ValueError(f"Unsupported patch contract: {args.patch_contract}")
...
def validate_hybrid_delta_payload(
    payload: dict[str, Any],
    schema: dict[str, Any],
    registry: dict[str, Any],
) -> tuple[list[str], list[str], dict[str, Any], list[str]]:
    ...
    actual_keys = list(payload.keys())
    if actual_keys != ["explanation", "changes"]:
        schema_errors.append("Root keys must be exactly ['explanation', 'changes'] in that order.")

    explanation_schema = schema.get("properties", {}).get("explanation", {})
    explanation = payload.get("explanation")
    if not isinstance(explanation, str):
        schema_errors.append("explanation must be a string.")
    else:
        min_length = explanation_schema.get("minLength")
        max_length = explanation_schema.get("maxLength")
        if min_length is not None and len(explanation) < min_length:
            schema_errors.append(f"explanation must be at least {min_length} characters.")
        if max_length is not None and len(explanation) > max_length:
            schema_errors.append(f"explanation must be at most {max_length} characters.")

    compact_payload = {"changes": payload.get("changes")}
    compact_schema_errors, range_errors, changed_paths, semantic_errors = validate_compact_delta_payload(
        compact_payload,
        registry,
    )
    for error in compact_schema_errors:
        if error == "Root keys must be exactly ['changes'], received ['changes'].":
            continue
...
def normalize_patch_payload(
    *,
    ...
    if patch_contract == PATCH_CONTRACT_HYBRID:
        schema_errors, range_errors, changed_paths, semantic_errors = validate_hybrid_delta_payload(
            payload,
            patch_schema,
            registry,
        )
    ...
    metrics = compute_changed_path_metrics(
        ...
        registry=registry,
        patch_contract=PATCH_CONTRACT_COMPACT,
    )
...
def apply_sanitized_changes(
    *,
    ...
    metrics = compute_changed_path_metrics(
        ...
        ignored_paths=ignored_paths,
        registry=registry,
        patch_contract=PATCH_CONTRACT_COMPACT if patch_contract == PATCH_CONTRACT_HYBRID else patch_contract,
    )
    sparse_fidelity = evaluate_sparse_fidelity(
        prompt_case=prompt_case,
        changed_paths=sanitized_changed_paths,
...
def default_patch_system_prompt(patch_contract: str, registry: dict[str, Any]) -> str:
    ...
    if patch_contract == PATCH_CONTRACT_HYBRID:
        header = [
            "You are the Sound Forge Experiment 1.3 hybrid patch engine.",
            "Return exactly one JSON object and nothing else.",
            "Do not add markdown, code fences, comments, or extra prose.",
            "The JSON object must have exactly two keys in this exact order: explanation, changes.",
            "explanation must briefly explain the intended acoustic logic for the patch request.",
            ...
            "If nothing should change, return an explanation and {\"changes\":[]}.",
            "Supported paths and value families:",
        ]
    else:
        header = [
            "You are the Sound Forge Experiment 1.3 compact delta patch engine.",
            "Return exactly one JSON object and nothing else.",
            "Do not add markdown, code fences, comments, or extra prose.",
...
def initialize_record(
    *,
    ...
    return {
        ...
        "patch_prompt_token_sanity_pass": None,
        "patch_prompt_token_sanity_error": None,
...
def patch_worker_loop(
    *,
    ...
    while True:
        ...
        try:
            if args.disable_patch_grammar:
                with patch_handle.lock:
                    response = patch_handle.llm.create_chat_completion(
                        messages=messages,
        ...
        if args.min_patch_prompt_tokens is not None:
            prompt_tokens = record.get("patch_prompt_tokens")
            if prompt_tokens is None or prompt_tokens < args.min_patch_prompt_tokens:
                record["patch_prompt_token_sanity_pass"] = False
                record["patch_prompt_token_sanity_error"] = (
                    f"Patch prompt token sanity check failed: expected >= {args.min_patch_prompt_tokens}, received {prompt_tokens}."
                )
                record["raw_validation_errors"] = [record["patch_prompt_token_sanity_error"]]
                ...
                continue
            record["patch_prompt_token_sanity_pass"] = True
        ...
        if patch_contract == PATCH_CONTRACT_HYBRID:
            record["raw_inline_explanation"] = parsed_payload.get("explanation")
...
def build_batch_payload(
    *,
    ...
    return {
        "experiment": "Experiment 1.3: The Convergence Sandbox",
        "phase": "phase-1-contracts-and-compatibility",
        "run_id": run_id,
        "run_state": run_state,
...
def save_latency_csv(path: Path, records: list[dict[str, Any]]) -> None:
    fieldnames = [
        ...
        "patch_prompt_tokens",
        "patch_prompt_token_sanity_pass",
        "patch_prompt_token_sanity_error",
            ...
            writer.writerow(
                {
                    ...
                    "patch_prompt_token_sanity_pass": (
                        "" if record["patch_prompt_token_sanity_pass"] is None else record["patch_prompt_token_sanity_pass"]
                    ),
                    "patch_prompt_token_sanity_error": record["patch_prompt_token_sanity_error"] or "",
...
def main() -> int:
    ...
    reporter.emit("Starting Experiment 1.3: The Convergence Sandbox")
    ...
    if args.patch_chat_format is not None:
        reporter.emit(f"Patch chat format: {args.patch_chat_format}")
    if args.min_patch_prompt_tokens is not None:
        reporter.emit(f"Patch prompt token sanity minimum: {args.min_patch_prompt_tokens}")

    patch_grammar = None
    if args.disable_patch_grammar:
        reporter.emit("Patch grammar disabled for compatibility smoke test.")
    else:
